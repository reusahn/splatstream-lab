...
OUT = Path('/content/outputs/neuralscene_bench_garden')
...
def main() -> int:
    ...
    # Match the Bonsai parser/eval protocol exactly, changing only the scene.
    # `images` is the logical COLMAP image root; Nerfstudio maps it to images_2
    # when --downscale-factor 2 is supplied and scales camera intrinsics accordingly.
    cmd = [
        str(NS_TRAIN),
        'splatfacto',
        '--output-dir', str(OUT),
    ...
    print('=== NeuralScene Bench: Garden / Splatfacto / 5K ===', flush=True)
