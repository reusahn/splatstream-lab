...
ROOT = Path('/content/outputs/neuralscene_bench_bonsai/bonsai-bench/splatfacto')
OUT = Path('/content/outputs/neuralscene_bench_bonsai/splatfacto_5k_eval.json')
RENDERS = Path('/content/outputs/neuralscene_bench_bonsai/splatfacto_5k_eval_renders')
...
def main() -> int:
    ...
    if not ROOT.exists():
        raise FileNotFoundError(f'Missing Splatfacto output root: {ROOT}')
    ...
    if not candidates:
        raise RuntimeError('No completed Splatfacto run with a checkpoint was found.')
    ...
    print('\n=== SPLATFACTO 5K HELD-OUT RESULTS ===')
